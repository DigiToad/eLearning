<script>
  ...
  function validateAll(fd) {
    ...
    if (!priceRaw && priceRaw !== "0") errs.price = "Price is required.";
      ...
      errs.price = "Price seems too high. Max ₹10,00,000.";
    ...
    if (!keywords) errs.keywords = "At least one SEO keyword is required.";
      ...
      const kws = keywords
        ...
        .filter(Boolean);
      if (kws.length < 2)
        errs.keywords = "Add at least 2 keywords (comma-separated).";
      if (kws.some((k) => k.length > 50))
        errs.keywords = "Each keyword must be under 50 characters.";
    }
    fd.set("title", title);
  ...
  function validateField(name, value) {
    ...
    if (name === "keywords") {
      ...
      if (!v) {
        ...
        if (kws.length < 2)
          errs.keywords = "Add at least 2 keywords (comma-separated).";
        else if (kws.some((k) => k.length > 50))
          errs.keywords = "Each keyword must be under 50 characters.";
  ...
  // ── Track current field values for dirty check ─────────────────────────
  ...
  $: isDirty = isEdit
    ...
    : true; // create mode always enabled
</script>

<div
  class=" bg-[#0a0c10] text-zinc-100 font-sans flex items-start justify-center py-12 px-4 rounded-md"
>
  <!-- <div class="pointer-events-none fixed inset-0 overflow-hidden">
    <div
      class="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full bg-violet-700/10 blur-[120px]"
    ></div>
    <div
      class="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full bg-indigo-600/8 blur-[100px]"
    ></div>
  </div> -->
  ...
  <div class="relative w-full max-w-2xl">
    ...
    <div class="mb-8">
      <div class="flex items-center gap-2 mb-3">
        ...
        <p
          class="text-[0.65rem] font-bold tracking-[0.2em] uppercase text-violet-400 font-mono"
        >
          Course Manager
        </p>
      </div>
    ...
    <div
      class="bg-[#12151c] border border-white/[0.06] rounded-3xl overflow-hidden shadow-2xl shadow-black/40"
    >
      <form
        method="POST"
      ...
      >
        ...
        <div class="px-8 py-7 space-y-5">
          <p
            class="text-[0.65rem] font-bold tracking-[0.18em] uppercase text-zinc-500 font-mono"
          >
            Course Basics
          </p>
          ...
          <div data-error={!!clientErrors.title}>
            <div class="flex justify-between items-center mb-1.5">
              <label
                class="text-[0.72rem] font-semibold tracking-widest uppercase text-zinc-400"
                for="f-title"
              >
                Course Title <span class="text-red-400">*</span>
              </label>
              <span class="text-[0.65rem] font-mono text-zinc-600"
                >{titleLen}/120</span
              >
            </div>
            <input
          ...
          <div class="grid grid-cols-2 gap-4">
            <div data-error={!!clientErrors.instructor}>
              <label
                class="block text-[0.72rem] font-semibold tracking-widest uppercase text-zinc-400 mb-1.5"
                for="f-inst"
              >
              ...
              <input
                ...
                class="block w-full bg-zinc-900/80 border rounded-xl px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600
                       outline-none transition-all focus:ring-2 focus:ring-violet-500/20
                       {clientErrors.instructor
                  ? 'border-red-500/50'
                  : 'border-white/8 focus:border-violet-500/50'}"
              />
              {#if clientErrors.instructor}
                <p class="text-red-400 text-xs mt-1.5 flex items-center gap-1">
                  <span>⚠</span>{clientErrors.instructor}
                </p>
              {/if}
            </div>
                ...
                >
                  ...
                  {#each ["Beginner", "Intermediate", "Advanced"] as lvl}
                    <option
                      value={lvl}
                      class="bg-zinc-900"
                      selected={iv("level") === lvl}>{lvl}</option
                    >
                  {/each}
                </select>
                <span
                  class="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-500 text-xs"
                  >▾</span
                >
              </div>
              {#if clientErrors.level}
                <p class="text-red-400 text-xs mt-1.5 flex items-center gap-1">
                  <span>⚠</span>{clientErrors.level}
                </p>
              {/if}
            </div>
          </div>
          ...
          <div data-error={!!clientErrors.description}>
            <div class="flex justify-between items-center mb-1.5">
              ...
              <span
                class="text-[0.65rem] font-mono {descLen > 480
                  ? 'text-amber-400'
                  : 'text-zinc-600'}">{descLen}/500</span
              >
            </div>
            <textarea
              id="f-desc"
            ...
            {#if clientErrors.description}
              <p class="text-red-400 text-xs mt-1.5 flex items-center gap-1">
                <span>⚠</span>{clientErrors.description}
              </p>
            {/if}
          </div>
        </div>
        ...
        <div class="px-8 py-7">
          <p
            class="text-[0.65rem] font-bold tracking-[0.18em] uppercase text-zinc-500 font-mono mb-5"
          >
            Thumbnail
          </p>
        ...
        <div class="px-8 py-7 space-y-5">
          ...
          <div data-error={!!clientErrors.price}>
            ...
            <div class="relative">
              <span
                class="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 text-sm font-semibold select-none"
                >₹</span
              >
              <input
                id="f-price"
          ...
          <div data-error={!!clientErrors.keywords}>
            ...
            <input
              ...
              class="block w-full bg-zinc-900/80 border rounded-xl px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600
                     outline-none transition-all focus:ring-2 focus:ring-violet-500/20
                     {clientErrors.keywords
                ? 'border-red-500/50'
                : 'border-white/8 focus:border-violet-500/50'}"
            />
            <p class="text-zinc-600 text-xs mt-1.5">
              Separate with commas. At least 2 required.
            </p>
            {#if keywordTags.length > 0}
              <div class="flex flex-wrap gap-1.5 mt-2.5">
                {#each keywordTags as tag}
                  <span
                    class="text-xs bg-violet-950/50 border border-violet-700/30 text-violet-300 px-2.5 py-1 rounded-lg font-mono"
                    >{tag}</span
                  >
                {/each}
              </div>
            ...
            {#if clientErrors.keywords}
              <p class="text-red-400 text-xs mt-1.5 flex items-center gap-1">
                <span>⚠</span>{clientErrors.keywords}
              </p>
            {/if}
          </div>
        </div>
        ...
        <!-- Footer -->
        <div
          class="px-8 py-6 bg-zinc-900/30 flex items-center justify-between gap-4 flex-wrap"
        >
          <div class="flex items-center gap-1.5 text-xs text-zinc-600">
            <span class="text-red-400">*</span><span
              >All starred fields are required</span
            >
          </div>
          <div class="flex items-center gap-3">
            <button
              ...
              on:click={() => {
                ...
                touchedFields = {};
                imageValue = course?.image ?? "";
                keywordsRaw = iv("keywords");
                titleLen = iv("title").length;
                descLen = iv("description").length;
              }}
              class="text-sm text-zinc-500 hover:text-zinc-300 px-5 py-2.5 rounded-xl hover:bg-white/5 transition-all"
              >Reset</button
            >
            <button
              type="submit"
