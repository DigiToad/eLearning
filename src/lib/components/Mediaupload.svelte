<script>
    ...
    async function handleFileChange(event) {
        ...
        if (isImage) {
            ...
            try {
                ...
                ready = false;
                toast.success(
                    `Image ready (${(file.size / 1024).toFixed(2)} KB) — click Process`,
                );
            } catch {
            ...
            if (selectedFile.size > 2 * 1024 * 1024 * 1024) {
                error = "Video must be less than 2 GB";
                toast.error(error);
                return;
            }
            file = selectedFile;
            ...
            ready = false;
            toast.success(
                `Video selected (${(file.size / (1024 * 1024)).toFixed(2)} MB) — click Process`,
            );
        }
    }
    ...
    async function uploadFile() {
        if (!file) {
            toast.warning(`Please select a ${label.toLowerCase()} first.`);
            return;
        }
        ...
        try {
            ...
            toast.error(
                `An error occurred while uploading the ${label.toLowerCase()}.`,
            );
            uploaded = false;
            ready = false;
        }
...
<div class="space-y-3">
    ...
    <div
        class="inline-flex rounded-md border border-gray-600 overflow-hidden text-sm"
    >
        <button
            type="button"
            on:click={() => switchInputType("file")}
        ...
        {#if previewUrl}
            ...
            {:else}
                ...
                <video
                    src={previewUrl}
                    controls
                    class="mt-2 rounded max-h-48 w-auto border border-white/10"
                ></video>
            {/if}
        {/if}
