<script>
  ...
  const startTimer = () => {
    ...
    timerInterval = setInterval(() => {
      if (timeLeft > 0) { timeLeft -= 1; } else { clearInterval(timerInterval); }
    }, 1000);
  };
  ...
  const handleResendOtp = () => {
    if (timeLeft === 0 && !isLoading) { form4.requestSubmit(); startTimer(); }
  };
</script>
    ...
    <div class="md:w-2/5 w-full flex flex-col justify-between bg-primary-500 text-white p-8 md:p-10 relative overflow-hidden">
      ...
      <div class="flex flex-col gap-3 relative z-10">
        <div class="bg-white/10 border border-white/20 rounded-2xl p-4">
          <div class="bg-white/15 rounded-lg w-9 h-9 flex items-center justify-center mb-3">
            <Icon icon="heroicons:command-line" class="text-white w-5 h-5" />
          </div>
          <h3 class="font-bold text-sm mb-1">Learn Through Play ✦</h3>
          <p class="text-white/70 text-xs leading-relaxed">Master coding with interactive challenges and rewards</p>
        </div>
        <div class="bg-white/10 border border-white/20 rounded-2xl p-4">
          <div class="bg-white/15 rounded-lg w-9 h-9 flex items-center justify-center mb-3">
          ...
          <p class="text-white/70 text-xs leading-relaxed">Get placed in top tech companies</p>
        </div>
      </div>
    </div>
          ...
          <form
            ...
            use:enhance={() => {
              ...
              return async ({ result, update }) => {
                ...
                if (result.type === "failure") { isLoading = false; toast.error(result.data.errorMsg); }
                else if (result.type === "success") { toast.success(result.data.errorMsg); startTimer(); }
                await applyAction(result);
                isLoading = false;
              };
            }}
          ...
          >
            ...
            <div class="space-y-1">
              ...
              <div class="relative">
                ...
                <input
                  ...
                  bind:value={email} on:input={validateEmail}
            ...
            {#if otpStatus !== 200}
              <button type="submit"
                ...
                disabled={isLoading}>
                {#if isLoading}
                  <Icon icon="line-md:loading-alt-loop" class="w-5 h-5 mr-2 animate-spin" /> Sending..
                {:else}
            ...
            {#if otpStatus === 200}
              <form action="?/verifyOtp" method="POST"
                use:enhance={({ formData }) => {
                  ...
                  return async ({ result }) => {
                    if (result.type === "failure") { toast.error(result.data.errorMsg); }
                    else if (result.type === "redirect") { await goto(result.location); location.reload(); }
                    else if (result.type === "success") {
                      const { redirectTo } = result.data;
                      if (redirectTo) { await goto(redirectTo); location.reload(); }
                      else { toast.error("Verification successful, but no redirect URL provided!"); }
                    }
                    await applyAction(result);
                  };
                }}
              ...
              >
                <div class="bg-primary-50 p-4 rounded-md border border-primary-100">
                  <p class="text-gray-700 text-sm mb-3">Enter the 6-digit OTP sent to your email.</p>
                  <div class="flex gap-2 items-center">
                    <div class="relative flex-1">
                      <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <Icon icon="heroicons:key" class="text-gray-400" />
                      </div>
                      <input type="text" maxlength="6" name="enteredOtp" bind:value={enteredOtp}
                        on:input={(e) => { enteredOtp = e.target.value.replace(/[^0-9]/g, "").trim(); }}
                        placeholder="6-digit OTP"
                        class="pl-10 w-full rounded-md border border-gray-300 bg-white p-2.5 text-sm focus:border-primary-500 focus:ring-0"
                      />
                    </div>
                    <button class="bg-primary-600 text-white text-sm py-2 px-4 rounded-md font-medium hover:bg-primary-700 transition-colors">
                      Verify
                    </button>
                  </div>
                </div>
              ...
              >
                {#if timeLeft > 0}
                  <div class="flex justify-center">
                    <span class="text-sm px-4 py-2 rounded-full bg-primary-50 text-primary-700 border border-primary-200 flex items-center">
                      <Icon icon="heroicons:clock" class="mr-2" /> Resend in {formatTime(timeLeft)}
                    </span>
                  </div>
                {:else}
                  <div class="flex justify-center mt-3">
                    <button on:click={handleResendOtp} type="submit"
        ...
        {:else}
          ...
          <form method="POST" action="?/login" use:enhance={handleFormSubmit} class="space-y-4">
            ...
            <div class="flex justify-end">
              <a href="/forgot" class="text-sm text-primary-600 hover:text-primary-700 hover:underline underline-offset-2">
                Forgot password?
              </a>
            </div>
