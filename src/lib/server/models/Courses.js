...
const CourseSchema = new mongoose.Schema({
  ...
  duration:        { type: String, default: '0h' },          // computed or entered manually
...
// Stores partial state while admin is going through the multi-step wizard
const CourseDraftSchema = new mongoose.Schema({
  courseId:      { type: String, required: true, unique: true },
  step:          { type: String, enum: ['info', 'modules', 'done'], default: 'info' },
  totalVideos:   { type: Number, required: true },            // declared by admin up front
  uploadedCount: { type: Number, default: 0 },
  courseData:    { type: mongoose.Schema.Types.Mixed }        // mirrors CourseSchema shape
}, {
  timestamps: true,
