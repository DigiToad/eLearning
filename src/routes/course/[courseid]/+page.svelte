<script>
	...
	let interestDone = false;
	let justSubmitted = false; // tracks in-session submission
		...
		try {
			...
			if (!res.ok) {
				...
				try {
					...
					// SvelteKit fail() responses wrap data in an array: [{error:1}, "message"]
					if (Array.isArray(json?.data)) {
						const msg = json.data.find(
							(d) => typeof d === "string",
						);
						if (msg) errMsg = msg;
					} else if (json?.error) {
	...
	function isLessonComplete(lessonId) {
		return !!(
			lessonProgress[lessonId]?.complete || serverLessonMap[lessonId]
		);
	}
		...
		try {
			...
			if (!isLessonComplete(lesId) && ct > maxReached + MAX_SEEK_DELTA) {
				...
				if (now - lastSkipWarnTime > 3000) {
					showToast(
						`You can only skip up to ${MAX_SEEK_DELTA}s ahead`,
						"info",
					);
	...
	function autoAdvance() {
		...
		if (activeLessonIdx < (mod.lessons || []).length - 1) {
			...
			else showToast("Get access to continue", "warning");
		} else {
...
{#if course}
	...
	>
		...
		>
			...
			<span
				class="text-[13px] font-medium text-[#d0cee0] flex-1 overflow-hidden text-ellipsis whitespace-nowrap"
				>{course.title}</span
			>
			...
			{#if hasAccess}
				<div
					class="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-full py-1 pl-1.5 pr-2.5"
				>
					<div
		...
		>
			...
			<main class="flex-1 overflow-y-auto flex flex-col scroll-smooth">
				{#if !currentLesson && !quizOpen}
					...
					<div class="relative overflow-hidden">
						...
						{:else}
							<div
								class="w-full h-[280px] bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]"
							></div>
						{/if}
						<div
							class="absolute inset-0 bg-gradient-to-t from-[#0d0d11] via-[#0d0d11]/50 to-transparent flex flex-col justify-end px-10 py-8"
						>
							{#if course.level}
								<span
									class="inline-block text-[10px] font-bold tracking-[0.12em] uppercase text-violet-400 bg-violet-400/12 border border-violet-400/20 px-2.5 py-[3px] rounded-full mb-3 w-fit"
								>
									{course.level}
								</span>
							{/if}
							<h1
								class="text-[28px] font-bold tracking-[-0.02em] leading-[1.2] text-[#f0eeff] mb-2"
							>
								{course.title}
							</h1>
							<p
								class="text-sm text-[#9ca3af] max-w-[580px] leading-relaxed mb-3.5"
							>
								{course.description ?? ""}
							</p>
							...
							<div class="flex flex-wrap gap-4 mb-5">
								{#if course.instructor}
									<span
										class="flex items-center gap-1.5 text-xs text-[#6b7280]"
									>
										<svg
											width="14"
											height="14"
								...
								{#if course.price}
									<span
										class="flex items-center gap-1.5 text-xs text-violet-400 font-semibold"
										>₹{course.price}</span
									>
								{/if}
							</div>
							...
							{#if !hasAccess}
								<div
									class="flex items-center gap-3.5 bg-amber-500/7 border border-amber-500/18 rounded-xl px-4 py-3 max-w-[520px] flex-wrap"
								>
									<div
										class="text-amber-500 opacity-80 flex-shrink-0"
									>
										<svg
											width="20"
											height="20"
									...
									<div class="flex-1 min-w-0">
										<strong
											class="block text-[13px] text-amber-300 mb-[2px]"
											>Preview mode</strong
										>
										<span class="text-[11px] text-[#6b7280]"
											>Unlock all {course.totalLessons ??
												""} lessons &amp; your certificate</span
										>
									</div>
									<!-- BUY BUTTON — login/interest logic -->
									{#if !authedUser}
										<a
											href="/login"
											class="flex-shrink-0 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold rounded-lg transition-colors whitespace-nowrap no-underline"
											>Login to Proceed</a
										>
									{:else}
										<!-- <button
								class="flex-shrink-0 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold rounded-lg transition-colors whitespace-nowrap cursor-pointer border-none"
								on:click={openInterestModal}
							>Interested in this course</button> -->
										{#if alreadyInterested}
											<button
												class="flex-shrink-0 px-4 py-2 bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 text-xs font-semibold rounded-lg cursor-default whitespace-nowrap"
					...
					<!-- Stats row -->
					<div
						class="grid gap-3 px-10 pt-7"
						style="grid-template-columns: repeat(auto-fit, minmax(160px, 1fr));"
					>
						<div
							class="bg-white/[0.03] border border-white/[0.07] rounded-[14px] p-4 flex items-center gap-3.5"
						>
							...
							{:else}
								<div
									class="w-11 h-11 rounded-full bg-[#3b0764] text-violet-300 flex items-center justify-center text-base font-bold flex-shrink-0"
								>
									{(course.instructor ?? "?")[0]}
								</div>
							{/if}
							<div>
								<p
									class="text-[10px] font-semibold tracking-[0.08em] uppercase text-[#4b5563] mb-0.5"
								>
									Instructor
								</p>
								<p class="text-sm font-medium text-[#d1d5db]">
									{course.instructor ?? "—"}
								</p>
							</div>
						</div>
						...
						>
							<div
								class="text-[32px] font-bold text-violet-400 leading-none"
							>
					...
					<div class="px-10 py-8 pb-10">
						<h2
							class="text-[11px] font-bold tracking-[0.1em] uppercase text-[#4b5563] mb-4"
						>
							Curriculum
						</h2>
						<div class="flex flex-col gap-2.5">
							{#each course.modules || [] as mod, mi}
								...
								<div
									class="bg-white/[0.025] border border-white/[0.07] rounded-[14px] overflow-hidden {accessible
										? ''
										: 'opacity-55'}"
								>
									<div
										class="flex items-center gap-3 px-[18px] py-3.5 border-b border-white/[0.05]"
									>
										<div
									...
									{modProg?.complete
												...
												: accessible
													? 'bg-[#3b0764] text-violet-300'
													: 'bg-white/[0.06] text-[#4b5563]'}"
										>
											{#if modProg?.complete}
												<svg
													width="12"
										...
										<div class="flex-1 min-w-0">
											<p
												class="text-[13px] font-semibold text-[#e5e7eb] truncate"
											>
												{mod.title}
											</p>
											<p
												class="text-[11px] text-[#4b5563] mt-0.5"
											>
										...
										{#if !accessible}
											<span
												class="text-[10px] text-[#6b7280] bg-white/[0.04] border border-white/[0.08] px-2 py-[2px] rounded-md"
												>Locked</span
											>
										{/if}
									</div>
									<div class="py-1.5">
										{#each mod.lessons || [] as les, li}
											...
											<div
												class="flex items-center gap-2.5 px-[18px] py-[7px] {accessible &&
												isPlayable
													? ''
													: 'opacity-40'}"
											>
											...
											{done
												...
												<span
													class="flex-1 text-xs {done
														? 'text-emerald-300'
														: 'text-[#9ca3af]'} overflow-hidden text-ellipsis whitespace-nowrap"
													>{les.title}</span
												>
												<span
													class="text-[10px] text-[#374151] flex-shrink-0"
												>
													{#if done}Done
													{:else if !isPlayable}
														{#if !authedUser}
															<a
																href="/login"
																class="text-amber-400 font-semibold no-underline hover:underline"
																>Login</a
															>
														{:else}
															<!-- <button class="text-violet-400 font-semibold bg-none border-none cursor-pointer p-0 text-[10px]" on:click={openInterestModal}>Interested</button> -->
														{/if}{#if alreadyInterested}
															<button
																class="flex-shrink-0 px-4 py-2 bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 text-xs font-semibold rounded-lg cursor-default whitespace-nowrap"
																disabled
																>✓ Request sent</button
														...
														{:else}
															...
															>
														{/if}
													{:else if les.type === "assessment"}{les
															.questions
															?.length ?? 0} Qs
													{:else if les.durationSeconds}{formatTime(
															les.durationSeconds,
														)}{/if}
												</span>
											</div>
				...
				{:else if quizOpen && currentLesson?.type === "assessment"}
					...
					<div class="px-10 py-8 max-w-[680px] w-full">
						<div class="flex items-start gap-3 mb-7">
							...
							<div>
								<h2
									class="text-xl font-bold text-[#f0eeff] tracking-[-0.01em]"
								>
									{currentLesson.title}
								</h2>
								<p class="text-xs text-[#4b5563] mt-1">
									...
									{#if totalQuizAttempts > 0}
										<span
											class="ml-2 text-[11px] text-amber-400 bg-amber-500/10 px-1.5 py-[1px] rounded"
						...
						{#if quizResult}
							<div
								class="rounded-2xl p-10 text-center border {quizResult.passed
									? 'bg-emerald-500/[0.06] border-emerald-500/20'
									: 'bg-red-500/[0.06] border-red-500/20'}"
							>
								<div
									class="text-[72px] font-extrabold tracking-[-0.04em] leading-none {quizResult.passed
										? 'text-emerald-400'
										: 'text-red-400'}"
								>
									{quizResult.score}<span class="text-[32px]"
										>%</span
									>
								</div>
								<p
									class="text-lg font-semibold mt-2 text-[#e5e7eb]"
								>
									{quizResult.passed
										? "Passed"
										: "Not passed"}
								...
								{#if !quizResult.passed}
									<button
										class="mt-6 px-7 py-2.5 bg-white/[0.07] hover:bg-white/[0.12] text-[#e5e7eb] text-[13px] font-semibold rounded-[10px] border-none cursor-pointer transition-all"
										on:click={retryQuiz}>Try again</button
									>
								{:else}
									<button
										class="mt-6 px-7 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-[13px] font-semibold rounded-[10px] border-none cursor-pointer transition-all"
						...
						{:else}
							<div class="flex flex-col gap-3.5">
								{#each questions as q, qi}
									<div
										class="bg-white/[0.025] border border-white/[0.07] rounded-[14px] p-5"
									>
										<p
											class="text-sm text-[#e5e7eb] leading-relaxed mb-3.5 flex items-start gap-2"
										>
											<span
												class="inline-flex items-center justify-center w-[22px] h-[22px] bg-[#3b0764] text-violet-300 text-[11px] font-bold rounded-full flex-shrink-0 mt-[1px]"
												>{qi + 1}</span
											>
											{q.question}
										</p>
										<div class="flex flex-col gap-2">
											{#each q.options as opt, oi}
												<label
													class="flex items-center gap-2.5 px-3.5 py-2.5 rounded-[10px] cursor-pointer transition-all
											{parseInt(quizAnswers[qi]) === oi
														? 'bg-violet-600/15 border border-violet-600/40'
												...
												>
													...
													/>
													<span
														class="w-4 h-4 rounded-full flex-shrink-0 transition-all border-2
												{parseInt(quizAnswers[qi]) === oi
															? 'border-violet-600 bg-violet-600'
															: 'border-white/20'}"
													></span>
													<span
														class="text-[13px] text-[#d1d5db]"
														>{opt}</span
													>
												</label>
											{/each}
					...
					<div class="flex flex-col">
						...
						>
							{#if videoLoading}
								...
								>
									<div
										class="w-9 h-9 rounded-full border-2 border-violet-600/30 border-t-violet-600 animate-spin"
									></div>
								</div>
							{/if}
							...
							{#if !hasAccess}
								<div
									class="absolute top-3 left-3 z-20 text-[11px] font-bold px-3 py-1 rounded-full bg-amber-500/90 text-black backdrop-blur-sm"
								>
									Free Preview
								</div>
							{/if}
						...
						<div class="px-7 py-5 border-b border-white/[0.06]">
							...
							>
								<div class="flex-1 min-w-0">
									<p
										class="text-[11px] font-semibold tracking-[0.05em] text-violet-600 uppercase mb-1.5"
									>
							...
							{#if currentLesson.durationSeconds && !isLessonComplete(currentLesson.id)}
								<div class="flex items-center gap-2.5 mt-3.5">
									<div
										class="flex-1 h-[3px] bg-white/[0.08] rounded-sm overflow-hidden"
									>
										<div
									...
									<span
										class="text-[11px] text-[#4b5563] font-semibold flex-shrink-0"
									>
										{Math.round(
											((lessonProgress[currentLesson.id]
												?.maxWatched ?? 0) /
								...
								<p class="text-[11px] text-[#374151] mt-1.5">
									Seek ±{MAX_SEEK_DELTA}s — full watch unlocks
									free seek
								</p>
							{:else if isLessonComplete(currentLesson.id)}
								<p class="text-[11px] text-emerald-900 mt-1.5">
									Completed — seek freely anywhere
								</p>
							{/if}
						</div>
						...
						{#if !hasAccess}
							<div
								class="flex items-center gap-3.5 mx-7 my-4 p-4 bg-amber-500/[0.05] border border-amber-500/15 rounded-[14px] flex-wrap"
							>
								...
								<div class="flex-1 min-w-[140px]">
									<strong
										class="block text-[13px] text-amber-300 mb-0.5"
										>Enjoying the preview?</strong
									>
									<span class="text-[11px] text-[#6b7280]"
										>Get all {course.totalLessons ?? ""} lessons
										+ certificate</span
									>
								</div>
								<!-- BUY BUTTON — login/interest logic -->
					...
					>
						...
						<div class="flex-1">
							...
							<p class="text-xs text-[#6b7280] mt-0.5">
								All modules finished — your certificate is ready
							</p>
						</div>
						<button
							class="flex-shrink-0 px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-bold text-[13px] rounded-[10px] border-none cursor-pointer transition-colors"
						>
			...
			{#if sidebarOpen}
				<aside
					class="w-[296px] flex-shrink-0 bg-[#101016] border-l border-white/[0.06] flex flex-col overflow-hidden"
				>
					<div
						class="px-4 py-4 border-b border-white/[0.06] flex items-center justify-between gap-3"
					>
						<div>
							<p
								class="text-xs font-bold text-[#e5e7eb] tracking-[0.02em]"
							>
								Course content
							</p>
						...
						{#if hasAccess}
							<div class="relative w-9 h-9 flex-shrink-0">
								...
								<span
									class="absolute inset-0 flex items-center justify-center text-[9px] font-bold text-violet-400"
									>{overallPct}</span
								>
							</div>
						{:else}
							<!-- Unlock All — login/interest logic -->
							{#if !authedUser}
								<a
									href="/login"
									class="px-3 py-1.5 bg-amber-500/12 border border-amber-500/25 text-amber-400 text-[11px] font-bold rounded-lg whitespace-nowrap no-underline transition-all hover:bg-amber-500/20"
									>Login</a
								>
							{:else}
								<!-- <button
							class="px-3 py-1.5 bg-violet-500/12 border border-violet-500/25 text-violet-400 text-[11px] font-bold rounded-lg whitespace-nowrap cursor-pointer border-none transition-all hover:bg-violet-500/20"
							on:click={openInterestModal}
						>Interested</button> -->
								{#if alreadyInterested}
									<button
					...
					<div
						class="flex-1 overflow-y-auto py-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/[0.07] [&::-webkit-scrollbar-thumb]:rounded-sm"
					>
						{#each course.modules || [] as mod, mi}
							{@const accessible = canAccessModule(mi)}
							{@const modProg = moduleProgress[mod.id]}
							...
							<div
								class="border-b border-white/[0.04] {accessible
									? ''
									: 'opacity-45'}"
							>
								<div
								...
								>
									<div
										class="w-[22px] h-[22px] rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0
								{modProg?.complete
											? 'bg-emerald-500/15 text-emerald-400'
											: accessible
												? 'bg-violet-600/20 text-violet-400'
												: 'bg-white/[0.04] text-[#374151]'}"
									>
										{#if modProg?.complete}
											<svg
												width="10"
									...
									<div class="flex-1 min-w-0">
										<p
											class="text-xs font-semibold text-[#d1d5db] truncate"
										>
											{mod.title}
										</p>
										<p
											class="text-[10px] text-[#374151] mt-[1px]"
										>
								...
								{#if accessible}
									<div class="pb-2">
										{#each mod.lessons || [] as les, li}
											...
											{isActive
														? 'bg-violet-600/10 border-l-violet-600'
											...
											{done
													...
													<span
														class="flex-1 text-[11.5px] overflow-hidden text-ellipsis whitespace-nowrap
											{done ? 'text-emerald-300' : isActive ? 'text-[#e5e7eb]' : 'text-[#9ca3af]'}"
													...
													<span
														class="text-[10px] text-[#374151] flex-shrink-0"
													>
														{#if isYouTube(les.videoUrl)}<span
																class="text-[9px] font-extrabold text-red-500 tracking-[0.04em]"
																>YT</span
															>
														{:else if done}✓
														{:else if les.type === "assessment"}{les
											...
											{:else}
												<div
													class="flex items-center gap-[9px] px-3.5 py-[7px] pl-5 opacity-40 border-l-2 border-l-transparent"
												>
													<div
														class="w-[18px] h-[18px] rounded-full bg-white/[0.04] flex items-center justify-center flex-shrink-0 text-[#374151]"
													>
														<svg
															width="8"
															height="8"
													...
													<span
														class="flex-1 text-[11.5px] text-[#9ca3af] overflow-hidden text-ellipsis whitespace-nowrap"
														>{les.title}</span
													>
													<!-- Locked lesson meta — login/interest logic -->
													{#if !authedUser}
														<a
															href="/login"
															class="text-[10px] text-amber-400 font-semibold no-underline hover:underline flex-shrink-0"
															>Login</a
														>
													{:else}
														<button
															class="text-[10px] text-violet-400 font-semibold bg-transparent border-none cursor-pointer p-0 flex-shrink-0"
															on:click|stopPropagation={openInterestModal}
															>Interested</button
														>
													{/if}
												</div>
		...
		>
			...
			>
				{#if !interestDone}
					...
					<h2
						class="text-xl font-bold text-[#f0eeff] mb-2.5 tracking-[-0.02em]"
					>
						Interested in this course?
					</h2>
					<p class="text-sm text-[#9ca3af] leading-relaxed mb-2">
						<strong class="text-[#e5e7eb]">{course?.title}</strong
						><br />
						{#if course?.price}<span
								class="text-violet-400 font-semibold"
								>₹{course.price}</span
							>{/if}
					</p>
					<p class="text-xs text-[#4b5563] mb-6">
						We'll note your interest and get back to you. Are you
				...
				{:else}
					...
					<p class="text-sm text-[#9ca3af] leading-relaxed mb-6">
						We've recorded your interest in <strong
							class="text-[#e5e7eb]">{course?.title}</strong
	...
	{#if toast}
		<div
			class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] px-5 py-[11px] rounded-xl text-[13px] font-medium text-white backdrop-blur-lg whitespace-nowrap shadow-[0_8px_24px_rgba(0,0,0,0.4)] pointer-events-none
		{toast.type === 'success'
				? 'bg-emerald-600/90 border border-emerald-500/30'
				: toast.type === 'error'
					? 'bg-red-800/90 border border-red-500/30'
					: toast.type === 'warning'
						? 'bg-orange-800/90 border border-amber-500/30'
						: 'bg-indigo-700/90 border border-indigo-500/30'}"
		>
			{toast.msg}
		</div>
	{/if}
