<script>
	...
	// ─── Match param name to your folder: [courseid] ────────────────
	...
	let lessonProgress = {};   // { [lessonId]: { maxWatched, complete } }
	let moduleProgress = {};   // { [moduleId]: { assessmentPassed, complete } }
	let overallPct = 0;
	...
	// ─── Progress: load from localStorage ───────────────────────────
	function loadProgress() {
		const saved = localStorage.getItem(`lms_progress_${courseId}`);
		if (saved) {
	...
	function saveProgress() {
		localStorage.setItem(
			...
			JSON.stringify({ lessonProgress, moduleProgress, quizAttempts })
		);
	}
	...
	function recalcProgress() {
		course.modules.forEach((mod) => {
			...
			moduleProgress[mod.id].complete = allLessonsDone && passed;
		});
	...
	function onVideoReady() {
		videoLoading = false;
		if (videoEl && currentLesson) {
			videoEl.currentTime = Math.min(maxReached, currentLesson.durationSeconds * 0.99);
			startHeartbeat();
		}
	}
	...
	function onSeeking() {
		if (!videoEl) return;
		if (videoEl.currentTime > maxReached + 2) {
	...
	function onTimeUpdate() {
		if (!videoEl || !currentLesson) return;
		if (videoEl.currentTime > maxReached) maxReached = videoEl.currentTime;
		if (!lessonProgress[currentLesson.id]?.complete && maxReached >= 0.95 * currentLesson.durationSeconds) {
	...
	function autoAdvance() {
		...
		if (activeLessonIdx < mod.lessons.length - 1) {
			...
			showToast('🎯 All lessons done! Take the quiz to unlock next module.', 'success');
		}
	}
...
<div class="min-h-screen bg-[#0f0f13] text-white flex flex-col">
	...
	<header class="h-14 border-b border-white/10 bg-[#12121a] flex items-center px-4 gap-4 shrink-0 z-30 sticky top-0">
		...
		<!-- Sidebar toggle -->
		<button on:click={() => (sidebarOpen = !sidebarOpen)} class="text-gray-400 hover:text-white transition ml-1">
	...
	<div class="flex flex-1 overflow-hidden" style="height: calc(100vh - 3.5rem);">
		...
		<main class="flex-1 overflow-y-auto flex flex-col">
			...
			<!-- ── Landing View (no lesson selected) ── -->
			{#if !currentLesson && !quizOpen}
				<div class="relative overflow-hidden">
					<img src={course.image} alt={course.title} class="w-full h-64 md:h-80 object-cover opacity-40" />
					<div class="absolute inset-0 bg-gradient-to-t from-[#0f0f13] via-[#0f0f13]/60 to-transparent flex flex-col justify-end p-8 md:p-12">
						<span class="text-violet-400 text-xs font-bold uppercase tracking-widest mb-2">{course.category}</span>
				...
				<div class="p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl">
					<div class="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center gap-4">
						<img src={course.instructorAvatar} alt={course.instructor} class="w-14 h-14 rounded-full bg-violet-900" />
						<div>
				...
				<div class="px-6 md:px-10 pb-10 flex flex-wrap gap-2">
					{#each course.tags as tag}
						<span class="bg-violet-900/40 border border-violet-700/30 text-violet-300 text-xs px-3 py-1 rounded-full">{tag}</span>
					{/each}
				</div>
			...
			{:else if quizOpen}
				...
				<div class="p-6 md:p-10 max-w-3xl w-full mx-auto">
					...
					<div class="flex items-center gap-3 mb-6">
						<button on:click={() => { quizOpen = false; quizResult = null; }} class="text-gray-400 hover:text-white transition">
							<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
							</svg>
						</button>
						<div>
							...
							<p class="text-sm text-gray-500">
								Pass mark: {assessment.passMark}% · Attempts used: {attempts}/{assessment.attemptsAllowed}
							</p>
						</div>
					</div>
					...
					{#if quizResult}
						<!-- Result -->
						<div class="rounded-2xl border p-8 text-center {quizResult.passed ? 'bg-emerald-900/30 border-emerald-500/30' : 'bg-rose-900/30 border-rose-500/30'}">
							<div class="text-6xl mb-4">{quizResult.passed ? '🎉' : '😔'}</div>
							<h3 class="text-2xl font-black mb-2">{quizResult.passed ? 'Passed!' : 'Not quite'}</h3>
							<p class="text-5xl font-black {quizResult.passed ? 'text-emerald-400' : 'text-rose-400'} mb-3">{quizResult.score}%</p>
							...
							{#if !quizResult.passed && attempts < assessment.attemptsAllowed}
								<button
									on:click={() => { quizResult = null; quizAnswers = {}; }}
					...
					{:else}
						...
						<div class="space-y-5">
							...
							<button
								...
								class="w-full py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 font-bold text-white
									disabled:opacity-40 disabled:cursor-not-allowed hover:from-violet-500 hover:to-violet-400 transition-all"
							>
								Submit Quiz
							</button>
						</div>
				...
				<div class="flex flex-col">
					...
					<div class="relative bg-black w-full" style="aspect-ratio:16/9; max-height:62vh;">
						...
						{#if lessonProgress[currentLesson.id]?.complete}
							<div class="absolute top-3 right-3 bg-emerald-600 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg z-10">
					...
					<div class="p-5 md:p-6 border-b border-white/10">
						<div class="flex items-start justify-between gap-4 flex-wrap">
							<div>
								<p class="text-xs text-violet-400 font-semibold uppercase tracking-wider mb-1">
									{currentModule.title} · Lesson {activeLessonIdx + 1}/{currentModule.lessons.length}
								</p>
								<h2 class="text-xl md:text-2xl font-bold">{currentLesson.title}</h2>
								<p class="text-sm text-gray-500 mt-1">
									Duration: {formatTime(currentLesson.durationSeconds)}
							...
							{#if activeLessonIdx < currentModule.lessons.length - 1}
								...
								>
									Next <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
								</button>
							{:else if canTakeAssessment(activeModuleIdx) && !moduleProgress[currentModule.id]?.assessmentPassed}
								<button
									on:click={openQuiz}
						...
						<div class="mt-4 flex items-center gap-3">
							...
							<span class="text-xs text-gray-500 shrink-0">
								{Math.round(((lessonProgress[currentLesson.id]?.maxWatched ?? 0) / currentLesson.durationSeconds) * 100)}% watched
							</span>
						</div>
						<p class="text-xs text-gray-600 mt-1.5">⏭ Seeking is locked beyond your furthest watched point</p>
					</div>
				</div>
			{/if}
			...
			<!-- Certificate Banner -->
			{#if certificateReady}
				<div class="m-6 p-5 rounded-2xl bg-gradient-to-r from-amber-900/40 to-orange-900/40 border border-amber-500/30 flex items-center gap-4 flex-wrap">
					<div class="text-5xl">🏆</div>
					<div class="flex-1">
						<h3 class="font-black text-xl text-amber-300">Course Complete!</h3>
						<p class="text-gray-400 text-sm mt-0.5">All modules and assessments done. Download your certificate below.</p>
					</div>
					<button class="shrink-0 px-5 py-2.5 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 transition text-sm">
		...
		<!-- ── Sidebar ─────────────────────────────────────────────── -->
		{#if sidebarOpen}
			<aside class="w-80 shrink-0 border-l border-white/10 bg-[#12121a] flex flex-col overflow-hidden">
				<div class="p-4 border-b border-white/10">
					<h3 class="font-bold text-sm">Course Content</h3>
					<p class="text-xs text-gray-500 mt-0.5">{course.modules.length} modules · {course.totalLessons} lessons</p>
					...
					{#each course.modules as mod, mi}
						...
						<div class="border-b border-white/5">
							...
							<div class="px-4 py-3 flex items-start gap-3 {accessible ? '' : 'opacity-50'}">
								<div class="w-6 h-6 rounded-full shrink-0 flex items-center justify-center mt-0.5
									{modProg?.complete ? 'bg-emerald-500' : accessible ? 'bg-violet-700' : 'bg-white/10'}">
									{#if modProg?.complete}
								...
								<div class="flex-1 min-w-0">
									...
									<p class="text-xs text-gray-500 mt-0.5">
										...
										{#if modProg?.assessmentPassed} · Quiz ✓{/if}
									</p>
								</div>
							</div>
							...
							{#if accessible}
								<div class="pb-2">
									...
									{@const canQuiz = canTakeAssessment(mi)}
									{@const quizPassed = moduleProgress[mod.id]?.assessmentPassed}
									<button
										on:click={() => { if (canAccessModule(mi)) { activeModuleIdx = mi; openQuiz(); } }}
										disabled={!canQuiz && !quizPassed}
										class="w-full flex items-center gap-3 px-4 py-2.5 text-left transition-all border-l-2
											{quizOpen && activeModuleIdx === mi
												? 'bg-amber-600/20 border-amber-500'
												: 'hover:bg-white/5 border-transparent'}
											{!canQuiz && !quizPassed ? 'opacity-40 cursor-not-allowed' : ''}"
									>
										<div class="w-5 h-5 rounded-full shrink-0 flex items-center justify-center
											{quizPassed ? 'bg-emerald-500' : 'bg-amber-600/30'}">
											{#if quizPassed}
												<svg class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
												</svg>
										...
										<div class="flex-1 min-w-0">
											<p class="text-xs font-medium {quizPassed ? 'text-emerald-400' : 'text-amber-400'} truncate">
												{assessment.title}
											</p>
											<p class="text-xs text-gray-600">
												{quizPassed ? 'Passed ✓' : canQuiz ? 'Ready to take' : 'Finish lessons first'}
											</p>
										</div>
									</button> -->
								</div>
...
<!-- Toast -->
{#if toast}
	<div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-xl text-sm font-semibold text-white shadow-2xl {toastColors[toast.type]} slide-up">
		{toast.msg}
	</div>
{/if}
